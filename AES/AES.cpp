// AES.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
void InvCipher()
{
	...
	//крч эта залупа рекурсивно вызывалась и хз нах она тут нужна
...
int main()
{
	...
	while (Nr != 128)
	{
		printf("Enter the length of Key(128): ");
		std::string inStr1;
		getline(std::cin, inStr1);
		Nr = std::stoi(inStr1);
	}
	...
	for (int i = 0; i < inStr.length() % 16; i++) {
		inStr += ' ';
	}

	const int countIter = inStr.length() / 16;

	unsigned char** matrix = new unsigned char* [countIter];
	for (i = 0; i < countIter; i++)
		matrix[i] = new unsigned char[16];

	printf("\nText after encryption:\n");
	for (int j = 0; j < countIter; j++) {

		for (i = 0; i < Nb * 4; i++) {
			in[i] = (char)inStr[j*16 + i];
		}
		KeyExpansion();
